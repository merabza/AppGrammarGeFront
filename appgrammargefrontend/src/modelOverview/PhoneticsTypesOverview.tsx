...
const PhoneticsTypesOverview: FC = () => {
  ...
  const phoneticsOptionsSorted = useMemo(() => {
    ...
    return phoneticsOptions2.sort((a, b) => a.phoName.localeCompare(b.phoName));
  }, [phoneticsOptions]);

  const phoneticsTypesSorted = useMemo(() => {
    if (!phoneticsTypes) return [];
    const phoneticsTypes2 = [...phoneticsTypes];
    return phoneticsTypes2.sort((a, b) => a.phtName.localeCompare(b.phtName));
  }, [phoneticsTypes]);
  ...
  return (
    <div>
      ...
      <h4>ტერმინები</h4>
      <p>
        <strong>ფონეტიკური ცვლილების ვარიანტი</strong> - ერთი ან რამდენიმე
        მოქმედების თანმიმდევრობა, რომელიც საჭიროა შესრულდეს ფუძის შესაცვლელად
        ერთ კონკრეტულ შემთხვევაში.
      </p>
      <p>
        <strong>ფონეტიკური ცვლილების ტიპი</strong> - ფონეტიკური ვარიანტები
        კომპლექტი, რომელიც დასაშვებია კონკრეტული ფუძისათვის
      </p>
      <p>
        <strong>ფონეტიკური ცვლილება</strong> - ფონეტიკური ტიპის კონკრეტული
        ფონეტიკური ვარიანტი
      </p>
      <p>
        <strong>ფონეტიკური ტიპის შეზღუდვა</strong> - იმისათვის, რომ ფონეტიკურ
        ტიპში შემავალმა ყველა ვარიანტმა იმუშავოს ზოგიერთ ფონეტიკურ ტიპს გააჩნია
        ერთი ან რამდენიმე შეზღუდვა
      </p>
      <p>
        <strong>ფონეტიკური ვარიანტის მოქმედება</strong> - ფონეტიკური ვარიანტის
        შესაბამისი ცვლილებების შესასრულებლად ჩასატარებელი ერთი კონკრეტული
        მოქმედება
      </p>
      ...
      <p>
        ფონეტიკური ვარიანტის სახელი ={">"} მოქმედებები ={">"} (აქ ჩაემატება
        რამდენჯერ არის გამოყენებული თითოეული ფონეტიკური ვარიანტი დერივაციაში და
        ერთი ან რამდენიმე ნიმუში დრივაციიდან)
      </p>
      <ol>
        {phoneticsOptionsSorted.map((pho) => {
          ...
          return (
            <li key={pho.phoId.toString()} ref={bl ? backLigth : null}>
              ...
              {phodActions.map((phoda, ind) => {
                ...
                return (
                  <span key={ind.toString()}>
                    {separator}
                    {GetOnephoneticsOptionDetailDescription(phoda)}
                  </span>
                );
              })}
            </li>
      ...
      {phoneticsTypesDataType.create && (
        <Link to={"/phoneticsTypeEdit"}>ახალი ფონეტიკური ტიპის შექმნა</Link>
      )}
      <p>
        ფონეტიკური ტიპის სახელი ={">"} ბოლო ბგერა ={">"} ხმოვნების რაოდენობა =
        {">"} (აქ ჩაემატება რამდენჯერ არის გამოყენებული თითოეული ფონეტიკური ტიპი
        სახელისა და ზმნის ფლექსიაში ერთი ან რამდენიმე ნიმუშის მოტანით)
      </p>
      ...
      <ol>
        {phoneticsTypesSorted.map((pht) => {
          ...
          if (pht.phtLastLetter === 1)
            ...
            lastLetText = "სულერთია ბოლო ბგერა ხმოვანი იქნება თუ თანხმოვანი";
          ...
          if (pht.phtSlabCount === 0) {
            ...
            if (pht.phtSlab !== 0) slabText = `არცერთი ხმოვანი არ უნდა იყოს`;
          }

          const phtProhibitions = phoneticsTypeProhibitions
            .filter((phtp) => {
              if (phtp.phoneticsTypeId !== pht.phtId) return false;
              if (phtp.phtpProhId === 1 && !phtp.phtpNew)
                //აქ გათვალისწინებულია, რომ არ შეიძლება იყოს ერთ-ერთი ცარელა ნიმუშით
                return false;
              if (phtp.phtpProhId !== 1 && !phtp.phtpNew && !phtp.phtpObject)
          ...
          if (phtProhibitions.length > 0) {
            prohText = phtProhibitions
              .map((phtp, ind) => {
                ...
                return (
                  separator + GetOnePhoneticsTypeProhibitionDescription(phtp)
                );
              })
              .join("");
          }
          ...
          return (
            <li key={pht.phtId} ref={bl ? backLigth : null}>
              ...
              {!phoneticsTypesDataType.delete &&
                !phoneticsTypesDataType.update &&
                pht.phtName}{" "}
              ={">"} {lastLetText} ={">"} {slabText} ={">"} {prohText} ={">"}{" "}
              აქვს შემდეგი ფონეტიკური ვარიანტები:
              <ul>
                {phoneticsChanges
                  .filter((ffc) => ffc.phoneticsTypeId === pht.phtId)
                  .sort(
                    (a, b) =>
                  ...
                  .map((fc, ind) => {
                    const phoar = phoneticsOptions.find(
                      (pho) => pho.phoId === fc.phoneticsOptionId
                    );
                    if (phoar)
                      return <li key={phoar.phoId}>{phoar.phoName}</li>;
                    return <div key={fc.phoneticsOptionId} />;
                  })}
              </ul>
            </li>
          );
