...
import type { InflectionVerbCompositionData } from "../TypesAndSchemas/InflectionVerbCompositionDataTypeAndSchema";
    ...
    const createOrUpdateInflectionVerbComposition = useCallback(
        async (
            ...
            if (dbrId) {
                ...
                ivcId =
                    InflectionVerbComposition.inflectionVerbComposition.ivcId;
            } else {
                createInflectionVerbComposition(InflectionVerbComposition);
                ivcId =
                    inflectionVerbCompositionForEdit?.inflectionVerbComposition
                        .ivcId;
            }

            //თავიდან ჩაიტვირთოს დერივაციის იდენტიფიკატორის მიხედვით ყველა საჭირო ძირი
            if (ivcId)
                await getRootByInflectionVerbCompositionId(ivcId).unwrap();
