...
const DerivationEdit: FC = () => {
    ...
    const setderivationFormula = useCallback(
        (
            ...
            const selectableRangeIds =
                derivationFormula.derivationFormulaDetails
                    .map((m) => {
                        const morpheme = morphemesQuery.find(
                            (f) => f.mrpId === m
                        ); //აქ იყო m.morphemeId თუ მარტო ეს ერთი რიცხვია დეტალებში, ასე უნდა დარჩერს
                    ...
                    .filter(
                        (f) =>
                            ...
                            ((f.range.mrSelectable &&
                                f.morpheme.mrpNom === -1) ||
                                (f.range.mrBaseNom !== null &&
                                    f.morpheme.mrpNom === 1))
                    )
                    .map((m) => m.range.mrId);
            ...
            const morphemeRangeIdsByDT = morphemeRangesByDerivationTypes
                .filter(
                    (f) =>
                        f.derivationTypeId ===
                        derivationFormula.derivationTypeId
                )
            ...
            newForm.derivationBranch.derivationFormulaId =
                newderivationFormulaId;
                        ...
                        <div>
                            ...
                            {/* <PhoneticsCombEditor
                                ...
                                onTrashButtonClick={(index) => {
                                    ...
                                    newFrm.basePhoneticsCombDetails.splice(
                                        index,
                                        1
                                    );
                                    setFormData(newFrm);
