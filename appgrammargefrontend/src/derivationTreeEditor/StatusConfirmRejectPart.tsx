...
const StatusConfirmRejectPart: FC<StatusConfirmRejectPartProps> = (props) => {
  ...
  switch (recordStatusId) {
    ...
    case 2:
      ...
      buttonRow[0] = { visible: false, buttonData: [] as ButtonDataModel[] };
      break;
    default:
      break;
  }
    ...
    <div>
            ...
            <Form.Group key={btnRowId} className="mb-1" as={Row}>
              <Col sm="10" align="right">
                {btnRow.buttonData.map((btn, btnId) => (
                  ...
                  >
                    <FontAwesomeIcon icon={btn.buttonIcon as IconName} />{" "}
                    {btn.buttonText}
                    {workingOnConfirmReject &&
      ...
      {buttonRow[buttonRowId].visible && (
        <MessageBox
          ...
          title={buttonRow[buttonRowId].buttonData[buttonId].messageTitle}
          text={buttonRow[buttonRowId].buttonData[buttonId].messageText}
          primaryButtonText="დიახ"
          secondaryButtonText="არა"
          onConfirmed={() => {
            setShowConfirmRejectMessage(false);
            onConfirmRejectClick(
              buttonRow[buttonRowId].buttonData[buttonId].opIsConfirm,
              buttonRow[buttonRowId].buttonData[buttonId].withAllDesc
            );
          }}
      ...
      {buttonRow[buttonRowId].visible && (
        <MessageBox
          ...
          title="შეცდომა"
          text={buttonRow[buttonRowId].buttonData[buttonId].errorMessagetext}
