...
import {
    type InflectionData,
    type InflectionPredecessorRedModel,
...
const InflectionEdit: FC = () => {
    ...
    //1. იდენტიფიკატორი
    const [curInfIdVal, setCurInfIdVal] = useState<number | undefined>(
        undefined
    );
    ...
    const setVerbType = useCallback(
        (curForm: InflectionData, newVerbTypeId: number) => {
            ...
            const verbPersonsCount = verbType
                ? verbType.vtpVerbPersonsCount
                : 0;
    ...
    const setInflectionType = useCallback(
        (curForm: InflectionData, newInflectionTypeId: number) => {
            ...
            ) as InflectionData; // { ...curForm } as InflectionData;
                            ...
                            <div>
                                ...
                                {frm.verb && (
                                    <div>
                                        ...
                                        {frm.personVariabilityTypeIds.length >
                                            0 && (
                                            <div>
                                                <OneStrongLabel
                                                ...
                                                {frm.personVariabilityTypeIds.map(
                                                    (m, i) => (
                                                        <OneComboBoxControl
                                                            key={i}
                                ...
                                <BasesAndFreeMorphemes
                                    ...
                                    onPredecessorChange={(baseNom, item) => {
                                        ...
                                        let predByBase =
                                            newForm.inflectionPredecessors.find(
                                                (pred) =>
                                                    pred.baseNom === baseNom
                                            );
                                        if (item) {
                                            if (!predByBase) {
                                                predByBase =
                                                    {} as InflectionPredecessorRedModel;
                                                predByBase.baseNom = baseNom;
                                                predByBase.parentBranchId =
                                                    item.itemId;
                                                predByBase.phoneticsTypeId =
                                                    null;
                                                newForm.inflectionPredecessors.push(
                                                    predByBase
                                                );
                                            } else {
                                                predByBase.parentBranchId =
