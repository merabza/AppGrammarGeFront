...
import { Morpheme, MorphemeRange } from "../masterData/mdTypes";
  ...
  morphemeIds &&
    morphemeIds.forEach((item) => {
      ...
      const rangeIndex = morpheme
        ? newRangesInGroup.findIndex((f) => f.mrId === morpheme.morphemeRangeId)
        : null;
      const range = rangeIndex != null ? newRangesInGroup[rangeIndex] : null;
  ...
  formData.morphemes.forEach((item, index, arr) => {
    ...
    if (
      ...
      const mrp2 = morphemesQuery.find(
        (f) => f.morphemeRangeId === range.mrId && f.mrpNom === newMrpNom
      );
      // console.log(
