...
const RootDerivationTree: FC = () => {
  ...
  function getBranches(branch: DerivationBranchModel) {
    ...
    return (
      <ul className="tree">
        {subVerbInflections.map((svi) => {
          const subInflectionVerbCompositions = inflectionVerbCompositions
            .filter((ivc) =>
          ...
          return (
            //autoscroll &&
            <li key={svi.infId} ref={inflBackLight ? backLigth : undefined}>
              <Link
                to={`/root/${currentRootId}/${branch.dbrId}/${svi.infId}`}
                className={inflBackLight ? "backLigth" : undefined}
              >
                <FontAwesomeIcon
                  icon="file-alt"
              ...
              <ul className="tree">
                {subInflectionVerbCompositions.map((sivc) => {
                  ...
                  return (
                    ...
                    <li
                      key={sivc.ivcId}
                      ref={ivcBackLight ? backLigth : undefined}
                    >
                      <Link
                        to={`/root/${currentRootId}/${branch.dbrId}/${svi.infId}/${sivc.ivcId}`}
                        className={ivcBackLight ? "backLigth" : undefined}
                      >
                      ...
                      <Link
                        ...
                        className={ivcBackLight ? "backLigth" : undefined}
                      >
                        {sivc.ivcName ? sivc.ivcName : "(---)"}{" "}
                        {sivc.ivcSamples ? `(${sivc.ivcSamples})` : ""}{" "}
                        {GetClassifierById(sivc.classifierId)}{" "}
                        {sivc.recordStatusId !== 2 && (
                          <StatusPart
                            recordStatusId={sivc.recordStatusId}
        ...
        {subNounInflections.map((sni) => {
          ...
          return (
            //autoscroll &&
            <li key={sni.infId} ref={inflBackLight ? backLigth : undefined}>
              <Link
          ...
          return (
            ...
            >
              <Link
                ...
                className={derivBackLight ? "backLigth" : undefined}
              >
      ...
      <Col md="6" className="editor-column">
        <div id="root-deriv-tree" className="editor-scroll">
          <h5 ref={rootBackLight ? backLigth : undefined}>
            ...
            >
              ძირი: {rootName}
              {rootHomonymIndex ? <sup>{rootHomonymIndex}</sup> : null}{" "}
              {rootNote} {GetClassifierById(classifierId)}{" "}
              {recordStatusId !== 2 && (
                <StatusPart recordStatusId={recordStatusId} creator={creator} />
              )}
            </Link>
          </h5>
          {getBranches(firstBranch)}
        </div>
      </Col>

      {!!currentInflId && <Paradigm InflectionIdentifier={currentInflId} />}
      {!!currentIvcId && (
        <Paradigm
