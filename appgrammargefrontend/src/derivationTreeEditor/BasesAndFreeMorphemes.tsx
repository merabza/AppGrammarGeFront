...
import TextBoxAutoComplete, {
    type ListElement,
    type listElementComponentProps,
} from "./TextBoxAutoComplete";
import BaseLink, { type BaseLinkElement } from "./BaseLink";
import type { FC } from "react";
import { useAppSelector } from "../appcarcass/redux/hooks";
import type { BaseLinkType, RootLink } from "../redux/types/rootsTypes";
...
import type { fnGetError } from "../appcarcass/hooks/useForman";
import type { DerivationPredecessorModel } from "./TypesAndSchemas/DerivationBranchDataTypeAndSchema";
import type { InflectionPredecessorRedModel } from "./TypesAndSchemas/InflectionDataTypeAndSchema";
                ...
                roots
                    .map((root) => {
                        return `${root.rootName}${
                            root.rootHomonymIndex > 0
                                ? `^${root.rootHomonymIndex}`
                                : ""
        ...
        <div>
            ...
            {ranges.map((range, index) => {
                ...
                const curDerivPre = predecessors
                    ? predecessors.find(
                          (depre) => depre.baseNom === range.mrBaseNom
                      )
                    : null;
                                ...
                                <TextBoxAutoComplete
                                    ...
                                    getTextByValue={(val) => {
                                        ...
                                        if (listItemBase) {
                                            ...
                                            text = countText(
                                                listItemBase.dbrBaseName,
                                                roots
                                            );
                                        } else {
                                            const preDerivBranch =
                                                getBranchByIdFromStore(
                                                    rootsRepo,
                                                    val
                                                );
