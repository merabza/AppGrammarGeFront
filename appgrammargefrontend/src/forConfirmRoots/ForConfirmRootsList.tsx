...
const ForConfirmRootsList: FC = () => {
  ...
  function createPageLinks() {
    ...
    if (
      ...
      if (currentPageNom > 1)
        pageLinks.push(
          createLink(rootStartsWith, createdUserName, currentPageNom - 1, "<")
        );
      for (let i = startWith; i < endWith; i++) {
        ...
        else pageLinks.push(createLink(rootStartsWith, createdUserName, i));
      }
    ...
    <div>
      <div className="mt-5 row">
        <h5>
          ...
          {!!rootStartsWith && (
            <span>
              რომლებიც იწყება ასოებით - <strong>{rootStartsWith}&nbsp;</strong>
            </span>
          )}
          {!!createdUserName && (
            <span>
                ...
                <li key={index}>
                  <Link to={`/root/${item.rootId}`}>
                    ...
                    {item.rootHomonymIndex > 0 && (
                      <sup>{item.rootHomonymIndex}</sup>
                    )}
                    {item.rootNote && <span> ({item.rootNote})</span>}
                  </Link>
                </li>
              );
            })}
