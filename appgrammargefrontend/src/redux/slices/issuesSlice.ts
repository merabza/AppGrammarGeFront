...
export const issuesSlice = createSlice({
    ...
    reducers: {
        ...
        saveIssueDetailLine: (
            state,
            action: PayloadAction<IIssueDetailLine>
        ) => {
        ...
        setCheckDetail: (
            state,
            action: PayloadAction<IcheckDetailParameters>
        ) => {
            const { issueId, detailsName, id, checkedValue } = action.payload;
            const detail = state.issuesDetailsRepo[issueId][
                detailsName
            ].rows.find((f) => f?.isdId === id);
            if (detail) {
        ...
        setIssuesTableFilterSortId: (
            ...
            if (tableName in state.filterSortIds)
                state.filterSortIds[tableName] =
                    state.filterSortIds[tableName] + 1;
